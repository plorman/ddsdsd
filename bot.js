...
| -role <name>  = لي اعطاء رتبة لشخص

| -roler <name>  = لي ازالة الرتبة من 

| -schannel = لي اغلاق الشات

| -schannel = لي فتح الشات
...
let welcomer = member.guild.channels.find("name","night");
      if(!welcomer) return;
      if(welcomer) {
         moment.locale('ar-ly');
         var h = member.user;
        ...
        .addField('`The date you entered DISCORD`',`${moment(member.user.createdAt).format('D/M/YYYY h:mm a')} **\n** \`${moment(member.user.createdAt).fromNow()}\``,true)
